...
def get_ready(task_set, cycle, ready_list):
    ...
    # sort on deadline 
    ready_list = sorted(ready_list, key=lambda
        t: t.deadline)  # https://www.geeksforgeeks.org/sorting-objects-of-user-defined-class-in-python/
...
def create_job(cycle,
               task):  # create new instance instead of deepcopy. Tried using list instead of task object. doesnt seem better and less readble
    new_job = Task(task.name, task.duration, task.period, task.type, task.priority, task.deadline + cycle,
                   et_subset=task.et_subset)
    # new_job.deadline = new_job.deadline + cycle
    new_job.release_time = cycle
...
def calculate_schedulability(polling_server):
    ...
    for et_task in et_tasks:
        ...
        while t <= hyperperiod:
            ...
            if supply >= demand:
                response_time = t
                result_dict[et_task.name] = (
                response_time, et_task.deadline)  # if actually greater than deadline, set to false later
                break
